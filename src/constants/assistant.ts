...
export const SCREEN_CONTEXT: Record<string, string> = {
  ...
  "template-selection": "The user is selecting a Terraform deployment template. AWS and GCP each have two templates: Standard (aws-simple, gcp-simple) and SRA (aws-sra, gcp-sra). Azure has three: Standard (azure-simple), Private Link (azure-pl-sts), and SRA (azure-sra). The Private Link template adds backend and DBFS private endpoints, private DNS zones, and serverless NCC without the hub-spoke/firewall/CMK complexity of SRA. SRA templates are currently grayed out with an 'IN DEVELOPMENT' badge and cannot be selected. Each card shows features.",
  "configuration": "The user is filling in Terraform template variables. Standard templates have: workspace name, region, networking (VPC/VNet/subnet CIDRs), tags, and optional existing VPC/VNet settings. The Azure Private Link template (azure-pl-sts) has: workspace name, resources prefix, region, create-or-use-existing resource group, VNet CIDR (restricted to /16-/24), two workspace subnet CIDRs (public + private, min /26), private endpoint subnet CIDR, service endpoints checkboxes (e.g. Microsoft.Storage, Microsoft.KeyVault), and tags. SRA templates have additional options: PrivateLink/PE/PSC configuration, CMK/CMEK encryption settings, compliance profiles, Security Analysis Tool (SAT), firewall rules (Azure), hub-spoke architecture with Databricks account resources like NCC and network policy (Azure), network hardening (GCP), and IP access lists (GCP). For Azure SRA, the 'Create Hub & Account Resources' toggle controls whether hub infrastructure and Databricks account-level resources (NCC, network policy, metastore) are created or must be provided as existing IDs. SAT configuration, Allowed FQDNs, and hub naming fields are only visible when hub creation is enabled (SAT is a hub-only feature). Values have validation rules.",
...
export const ASSISTANT_SAMPLE_QUESTIONS: Record<string, string[]> = {
  ...
  "template-selection": [
    ...
    "What does the Azure Private Link template do?",
    "Can I use an existing VPC?",
  ],
  "configuration": [
    "What CIDR ranges should I use?",
    "How do I configure VPC settings?",
    "What are service endpoints and should I enable them?",
